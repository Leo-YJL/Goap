...
namespace ReGoap.Unity {
    ...
    public class ReGoapPlannerThread<T, W> {
        ...
        /// 初始化
        /// </summary>
        /// <param name="plannerSettings"></param>
        /// <param name="onDonePlan"></param>
    ...
    public class ReGoapPlannerManager<T, W> : MonoBehaviour {
        ...
        [Header("Used only if MultiThred is set to ture")][Range(1,120)]
        ...
        protected virtual void Awake() {
            ...
            if (Instance != null) {
                Destroy(this);
                var errorString = "[GoapPlannerManager] Trying to instantiate a new manager but there can be only one per scene.";
            ...
            if (MultiThread) {
                //多个Planner
                ReGoapLogger.Log($"[GoapPlannerManager] Running in multi-thread mode ({ThreadsCount} threads).");
